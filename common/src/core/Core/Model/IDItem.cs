...
namespace VVVV.Core.Model
{
    public class IDItem : IIDItem, IDisposable
    {
        ...
        protected virtual object ObtainSymbol()
        {
            return null;
        }

        protected virtual void AcknowledgeNewSymbol()
        {
        }
        ...
        /// <summary>
        /// the semantic symbol is an immutable snapshot
        /// it reflects the source code together with the binding 
        /// </summary>
        public object Symbol
        {
            get
            {
                if ((FSymbol == null) || Changed)
                {
                    Changed = false;
                    FSymbol = ObtainSymbol();

                    if (FSymbol != null)
                        AcknowledgeNewSymbol();
                }
                return FSymbol;
            }
        }
        ...
        public virtual void MarkDefinitionChanged()
        {
            DefinitionChanged = true;

            // a new definition will lead to a new symbol. so mark symbol changed
            MarkChanged();
        }

        protected virtual object ObtainDefinition()
        {
            return null;
        }

        protected virtual void AcknowledgeNewDefinition()
        {
        }

        private object FDefintion;

        /// <summary>
        /// the semantic definition is an immutable snapshot of the binding from which the symbol is built of
        /// </summary>
        public object Defintion
        {
            get
            {
                if ((FDefintion == null) || DefinitionChanged)
                {
                    FDefintion = ObtainDefinition();

                    DefinitionChanged = false;

                    if (FDefintion != null)
                        AcknowledgeNewDefinition();
                }
                return FDefintion;
            }
        }
