...
float4 p0(float2 vp:vpos):color 
{
	...
	float2 uv = position;
    float lum = length(n);
	float3 tc = 1.-pow(float3(1.-lum, 1.-lum, 1.-lum),float3(r,g,b));
	return float4(tc.x,tc.y, tc.z, 1.0);
}

void vs2d(inout float4 vp:POSITION0,inout float2 uv:TEXCOORD0){vp.xy*=2;uv+=.5/R;}
technique Triolge{pass pp0{vertexshader=compile vs_3_0 vs2d();pixelshader=compile ps_3_0 p0();}}
