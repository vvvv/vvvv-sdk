...
#define sphstep(a,b,c) sqrt(1-pow(abs(c)*.01,2))
...
float4 p0(float2 vp:vpos):color{float2 x=(vp+.5)/R;float2 asp=lerp(1,R.x/R,Aspect);
    ...
    for (float i=0;i<1;i+=1./16){
        ...
        //float4 nc=tex2Dlod(s0,float4(x+off/48*Radius*asp,0,1));
    	//float k=pow(smoothstep(Threshold,0,distance(nc.rgb,cc.rgb)),3);
    	//c.rgb+=nc*k;
        //kk+=k;
    	float4 nc1=tex2Dlod(s0,float4(x+off/48*Radius*asp,0,1));
    	float4 nc2=tex2Dlod(s0,float4(x-off/48*Radius*asp,0,1));
    	
    	c.rg+=off*(dot(nc1.xyz,1./3.)-dot(nc2.xyz,1./3.));
    }
	c.rg=normalize(c.rg)*sphstep(0,1,pow(length(c.rg),1))*.5+.5;
	c.a=1;
  //  c=c/kk;
//	if(kk<2)c=cc;
//	c.a=cc.a;
