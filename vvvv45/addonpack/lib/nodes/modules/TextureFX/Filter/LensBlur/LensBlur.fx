...
float Gamma =5;
float Rand <float uimin=0.0; float uimax=1.0;> =1;
...
float4 pBLUR(float2 vp:vpos):color{float2 x=(vp+.5)/R;
	...
	float iter=12;
	float2 def=length(sin(x*718+wd*888+1882*tex2D(s0,x)))*.7+1*dot(vp%2,.03*float2(1,2));
	//def=0;
	for(float i=0;i<1;i+=1./max(0,min(Iterations,80))){
		float2 off=pow(i*.9999,.4)*sin((i*iter*8*.315*(1+Rand)+def+float2(.25,0))*acos(-1)*2);
		//off*=R.x/R;
