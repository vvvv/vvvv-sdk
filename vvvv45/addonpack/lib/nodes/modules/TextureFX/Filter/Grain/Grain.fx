...
float4 p0(float2 x:TEXCOORD0):color{
       float2 dx=x+tex2D(s0,x*1.5+.2)*.6;
    float4 c=float4(sin(dx.yx*28)+dx*dx.yx*2,length(x*2),sin(dx.x*12+dx.y*28))+tex2D(s0,dx+.2)*3+tex2D(s0,(dx.yx-.45)*.8+.45)*2+tex2D(s0,(dx-.5)*998.8+.5)+tex2D(s0,(dx.yx-.5)*798.8+.5+.31);
...
technique Grain{pass pp0{vertexshader=compile vs_2_0 vs2d();pixelshader=compile ps_2_0 p0();}}
