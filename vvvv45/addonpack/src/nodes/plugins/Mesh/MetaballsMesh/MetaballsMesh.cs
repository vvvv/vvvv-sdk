...
{
    [PluginInfo(Name = "Metaballs",
                Category = "EX9.Geometry",
                Author = "majortom, vvvv group",
                Help = "MetaballsMesh",
                Credits = "Base code ported from example by Andreas Jönsson @ www.AngelCode.com")]
    public class PluginMeshTemplate : IPluginEvaluate, IPartImportsSatisfiedNotification
    {
        ...
        [Input("Position", MinValue = -1, MaxValue = 1)]
        protected ISpread<Vector3> FPositionIn;
        [Input("Mass", MinValue = 0, MaxValue = 2, DefaultValue = 1)]
        protected ISpread<float> FMassIn;
        [Input("Grid Size", MinValue = 2, MaxValue = 128, DefaultValue = 2, IsSingle = true)]
        protected IDiffSpread<int> FGridSizeIn;
        [Input("Level", DefaultValue = 1, IsSingle = true)]
        protected IDiffSpread<float> FLevelIn;
        [Input("Smooth Mesh", IsSingle = true)]
        protected IDiffSpread<bool> FSmoothIn;

        //output pin declaration
        [Output("Mesh")]
        protected ISpread<MeshResource<Metaball>> FMeshOut;
        ...
        public void Evaluate(int spreadMax)
        {
            ...
            var update = false;
            if (FGridSizeIn.IsChanged)
            {
                var gridSize = FGridSizeIn[0];
                ...
                VoxelSize = 2 / (float)gridSize;
                GridSize = gridSize;
                update = true;
            }
            if (FLevelIn.IsChanged)
                update = true;
            if (FSmoothIn.IsChanged)
                update = true;

            // Check parameters per metaball
            FMeshOut.ResizeAndDispose(spreadMax, CreateMeshResource);
            for (int i = 0; i < spreadMax; i++)
            {
                var resource = FMeshOut[i];
                var metaball = resource.Metadata;
                var position = FPositionIn[i];
                if (position != metaball.Position)
                {
                    metaball.Position = position;
                    update = true;
                }
                var mass = FMassIn[i];
                if (mass != metaball.Mass)
                {
                    metaball.Mass = mass;
                    update = true;
                }
            ...
            if (update)
            {
                ...
                if (FSmoothIn[0]) SmoothMesh();

                // compute normals for all vertices
                ComputeNormals();

                // Force recreation of resources
                foreach (var meshResource in FMeshOut)
        ...
        protected void Render()
        {
            ...
            {
                var metaball = FMeshOut[i].Metadata;
                x = World2Grid(metaball.Position.X);
                y = World2Grid(metaball.Position.Y);
                z = World2Grid(metaball.Position.Z);
        ...
        {
            var numBalls = FMeshOut.SliceCount;
            var fEnergy = 0f;
            for (int i = 0; i < numBalls; i++)
            {
                ...
                var metaball = FMeshOut[i].Metadata;
                var p = metaball.Position;
                var fSqDist = (p.X - x) * (p.X - x) +
                              (p.Y - y) * (p.Y - y) +
                              (p.Z - z) * (p.Z - z);

                if (fSqDist < 0.0001f) fSqDist = 0.0001f;

                fEnergy += metaball.Mass / fSqDist;
            }
            return fEnergy;
        }
        ...
        {
            var numBalls = FMeshOut.SliceCount;
            var buffer = FMeshOut.Stream.Buffer;
            for (int j = 0; j < NumVertices; j++)
            {
                ...
                for (int i = 0; i < numBalls; i++)
                {
                    ...
                    var metaball = buffer[i].Metadata;
                    var position = metaball.Position;
                    var mass = metaball.Mass;
                    float x = VxBuffer[j].Vel.X - position.X;
                    float y = VxBuffer[j].Vel.Y - position.Y;
                    float z = VxBuffer[j].Vel.Z - position.Z;

                    var f4Dist = (x * x + y * y + z * z);
                    f4Dist *= f4Dist;

                    normal.X += 2 * mass * x / f4Dist;
                    normal.Y += 2 * mass * y / f4Dist;
                    normal.Z += 2 * mass * z / f4Dist;
