...
namespace VVVV.Nodes
{
	...
	//class definition
	public class IntAdvNode: IPlugin, IDisposable
    {	          	
        ...
        protected virtual void Dispose(bool disposing)
        {
        	...
        	if(!FDisposed)
        	{
        		...
        		FHost.Log(TLogType.Debug, "IntAdvNode is being deleted");
        ...
        public static IPluginInfo PluginInfo
	    {
	        get 
	        {
	        	if (FPluginInfo == null)
				{
					...
					FPluginInfo.Help = "Integral (Spreads) with bin and vector size";
        ...
        public void SetPluginHost(IPluginHost Host)
	    {
	    	...
	    	FHost.CreateValueInput("Include 0 Slice", 1, null, TSliceMode.Dynamic, TPinVisibility.OnlyInspector, out FInclZero);
	    	FInclZero.SetSubType(0, 1, 1, 1, false, true, false);
        ...
        {     	
        	...
        	if (FInput.PinIsChanged || FVecSize.PinIsChanged || FBinSize.PinIsChanged || FInclZero.PinIsChanged)
        	{	
	        	double tmpVec;
	        	...
	        	for (int i=0; i<spread.BinCount; i++)
	        	{
	        		...
	        		double[] increment = new double[vecSize];
	        		double curIncl;
	        		FInclZero.GetValue(i, out curIncl);
	        		if (curIncl>0.5)
	        			outList.AddRange(increment);
	        		
	        		for (int j=0; j<(spread.GetBin(i).Count/vecSize); j++)
	        		{
	        			for (int k=0; k<vecSize; k++)
	        			{
	        				increment[k]+=spread.GetBinVector(i, j)[k];
	        				//if (j>0)
	        				outList.Add(increment[k]);
	        				//outList.Add(spread.GetBinVector(i, j+1)[k]+spread.GetBinVector(i, j)[k]);
	        			}
	        		}
	        	}
