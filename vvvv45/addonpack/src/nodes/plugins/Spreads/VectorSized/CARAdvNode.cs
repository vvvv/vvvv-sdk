...
namespace VVVV.Nodes
{
	...
	//class definition
	public class CARAdvNode: IPlugin, IDisposable
    {	          	
    	...
    	private IValueOut FFirstOut;
    	private IValueOut FRestOut;
        ...
        protected virtual void Dispose(bool disposing)
        {
        	...
        	if(!FDisposed)
        	{
        		...
        		FHost.Log(TLogType.Debug, "CARAdvNode is being deleted");
        ...
        public static IPluginInfo PluginInfo
	    {
	        get 
	        {
	        	if (FPluginInfo == null)
				{
					...
					FPluginInfo.Help = "CAR (Spreads) with bin and vector size";
        ...
        public void SetPluginHost(IPluginHost Host)
	    {
	    	...
	    	//create outputs	    	
	    	FHost.CreateValueOutput("First Slice", 1, null, TSliceMode.Dynamic, TPinVisibility.True, out FFirstOut);
	    	FFirstOut.SetSubType(double.MinValue, double.MaxValue, 0.01, 0, false, false, false);
	    	
	    	FHost.CreateValueOutput("Remainder", 1, null, TSliceMode.Dynamic, TPinVisibility.True, out FRestOut);
	    	FRestOut.SetSubType(double.MinValue, double.MaxValue, 0.01, 0, false, false, false);
        ...
        {     	
        	...
        	{	
	        	...
	        	{
	        		first.AddRange(spread.GetBinVector(i,0));
	        		for (int j=1; j<(spread.GetBin(i).Count/vecSize); j++)
	        		{
	        			rest.AddRange(spread.GetBinVector(i,j));
	        		}
	        	}
        		
	        	FFirstOut.SliceCount=first.Count;
	        	for (int i=0; i<first.Count; i++)
	        	{
	        		FFirstOut.SetValue(i, first[i]);
	        	}
	        	
	        	FRestOut.SliceCount=rest.Count;
	        	for (int i=0; i<rest.Count; i++)
	        	{
	        		FRestOut.SetValue(i, rest[i]);
	        	}
        	}      	
        }
