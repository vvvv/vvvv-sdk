...
namespace StructureSynth {
	namespace Model {
		...
		void Builder::recurseDepthFirst(int& maxTerminated, int& minTerminated, int& generationCounter) {
			int lastValue = 0;
		...
		void Builder::recurseBreadthFirst(int& maxTerminated, int& minTerminated, int& generationCounter) {
			int syncSeed = 0;
			if (syncRandom) {
		...
		void Builder::build() {
			...
			if (ruleSet->recurseDepthFirst()) {
				recurseDepthFirst(maxTerminated, minTerminated, generationCounter);
			} else {
				recurseBreadthFirst(maxTerminated, minTerminated, generationCounter);
			}
