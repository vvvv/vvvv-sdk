...
{
	#region PluginInfo
	[PluginInfo(Name = "R", Category = "Network.Interprocess.String", 
	            AutoEvaluate = false, 
	            Author="ft", Help = "S/R nodes that allow to communicate between different vvvv instances", Tags = "")]
	#endregion PluginInfo
	public class RStringNode : IPluginEvaluate, IDisposable
	{
		#region pins
...
//			Log(LogType.Debug, "[R.String] Received new message from " + sendingPartnerId + " of size " + msgLength);
			...
			try {
				...
				if (Utils.GetMessageType(bytes) == MessageTypeEnum.stringSpread) {
			...
			//Log(LogType.Debug, "[R.String] New spread has " + receivedSpread.SliceCount + " slices.");
...
//					debug += ( i > 0 ? " | " : "" ) + spread[i];
//				}
//				Log(LogType.Debug, "[R.String] New spread contains: " + debug );
//			} catch (Exception e) {
...
//			}
		...
		public void Evaluate(int SpreadMax) {
			if (FChannelIn.IsChanged) {
				...
				LogNow(LogType.Debug, "[R.String] New LocalMessageBroadcastPartner created with id = " + localMessageBroadcastPartner.PartnerId);
			...
			if ( received ) {
				...
				//LogNow(LogType.Debug, "[R.String] Try to update output" );
