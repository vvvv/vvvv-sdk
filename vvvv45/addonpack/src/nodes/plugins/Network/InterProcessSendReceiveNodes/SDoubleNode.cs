...
{
	#region PluginInfo
	[PluginInfo(Name = "S", Category = "Network.Interprocess.Double", 
	            AutoEvaluate = true, 
	            Author="ft", Help = "S/R nodes that allow to communicate between different vvvv instances", Tags = "")]
	#endregion PluginInfo
	public class SDoubleNode : IPluginEvaluate, IDisposable
	{
		#region pins
		...
		public void Evaluate(int SpreadMax) {
			if (FChannelIn.IsChanged) {
				...
				LogNow(LogType.Debug, "[S.Double] New LocalMessageBroadcastPartner created with id = " + localMessageBroadcastPartner.PartnerId);
			...
			if ( (FValueIn.IsChanged || newPartner) && localMessageBroadcastPartner != null) {
				...
				//LogNow(LogType.Debug, "[S.Double] Trying to send message of size " + msg.Length);
