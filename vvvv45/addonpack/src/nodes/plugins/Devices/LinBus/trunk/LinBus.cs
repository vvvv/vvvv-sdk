...
namespace VVVV.Nodes
{
    ...
    public class LinBus : IDisposable, IPlugin
    {
        ...
        public void SetPluginHost(IPluginHost Host)
        {
            ...
            FDataFieldSize.SetSubType(0, double.MaxValue, 2, 8, false, false, true);
        ...
        public void Evaluate(int SpreadMax)
        {
            ...
            if (FPortNumberIn.PinIsChanged)
            {
                if (_Port != null)
                {
                    _Port.StopThread = false;

                    while (_Port.ThreadIsAlive())
                        Thread.Sleep(1);
                ...
                try
                {
                    if (_Port == null)
                    {
                        ...
                        if ((new List<string>(SerialPort.GetPortNames())).Contains(String.Format("COM{0}", currentSlicePortNumber)))
                        {
                            ...
                            _Port = new Driver((int)currentSlicePortNumber, (int)currentSliceBaudrate, Parity.None, 8, StopBits.One);

                            _Port.SetParameters(currentSliceIndentifier, (int)currentSliceDataFieldSize, (int)currentTimeOutTime);
                    ...
                    else
                    {
                        ...
                        {
                            _Port.Baudrate = (int)currentSliceBaudrate;
                        }
        ...
        private void SendAndWait()
        {
            while (_StopThread)
            {
                try
                {
                    ...
                    while (_Port.BytesToRead < _BytesToWait && _StopWatch.ElapsedMilliseconds < _TimeOutTime)
                        Thread.Sleep(1);
