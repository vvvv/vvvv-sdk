...
namespace VVVV.Hosting.Streams
{
	...
	{
		class ColorOutStreamWriter : VectorOutStreamWriter
		{
			public ColorOutStreamWriter(ColorOutStream stream)
				: base(stream)
			{
			...
			public override int Write(RGBAColor[] buffer, int index, int length, int stride)
			{
				...
				{
					RGBAColor* dst = (RGBAColor*) FPointer;
		...
		{
			FRefCount++;
			return new ColorOutStreamWriter(this);
		}
	}
