...
{
    [ComVisible(false)]
    public class HostExportProvider: ExportProvider, IDisposable
    {
        #region PinExport
        class PinExport : Export
        {
            public PinExport(HostExportProvider exportProvider, string contractName, Func<object> exportedValueGetter)
                : base(contractName, () => ExportedPinGetter(exportProvider, exportedValueGetter))
            {
                
            }
            
            static object ExportedPinGetter(HostExportProvider exportProvider, Func<object> exportedValueGetter)
            {
                var exportedValue = exportedValueGetter();
                var disposableExportedValue = exportedValue as IDisposable;
                if (disposableExportedValue != null)
                {
                    exportProvider.FDisposableExports.Add(disposableExportedValue);
                }
            ...
            else if (contractName.StartsWith("VVVV.PluginInterfaces"))
            {
                ...
                foreach (var attribute in GetImportDefinitionAttributes(definition))
                {
                    ...
                    if (typeToExport.IsGenericType)
                    {
                        ...
                        {
                            yield return new PinExport(this,contractName, () => PinFactory.CreateDiffPin(PluginHost, attribute, genericArgumentType));
                        }
                        else if (typeof(ISpread<>).MakeGenericType(genericArgumentType).IsAssignableFrom(typeToExport))
                        {
                            yield return new PinExport(this,contractName, () => PinFactory.CreatePin(PluginHost, attribute, genericArgumentType));
                        }
        ...
        public void Dispose()
        {
            foreach (var disposable in FDisposableExports)
            {
                disposable.Dispose();
            }
            FDisposableExports.Clear();
        }
    }
}
