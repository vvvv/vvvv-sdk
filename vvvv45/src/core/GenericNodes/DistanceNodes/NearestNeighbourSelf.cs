...
{
    public abstract class NearestNeighbourSelf<T> : IPluginEvaluate
    {
        ...
        [Output("Distance")]
        protected ISpread<double> FDistance;
        ...
        public void Evaluate(int SpreadMax)
        {
            if (SpreadMax > 1)
            {
                ...
                this.FDistance.SliceCount = this.FInput.SliceCount;
                ...
                for (int i = 0; i < this.FInput.SliceCount; i++)
                {
                    ...
                    for (int j = 0; j < this.FInput.SliceCount; j++)
                    {
                        if (j != i)
                        {
                            T refobj = this.FInput[j];
